...
 
int dp[(1<<15)+2];
int damage[16][16],health[16];
int n;
 
int shots(int n,int k){
    if(k==0) return oo;
    if(k>=n) return 1;
    if(n%k==0) return n/k;
    return (n/k)+1;
...
        if(!check(mask,i)){
            int die;
            die=shots(health[i],1);
 
            REP(k,wep.sz) die=min(die,shots(health[i],damage[wep[k]][i]));
 
            vci temp=wep;
            temp.pb(i);
            rt=min(rt,die+bitmask(on(mask,i),temp));
        }
    }
    return rt;
}
...
    while(t--){
        sc1(n);
        REP(i,n) sc1(health[i]);
        REP(i,n){
            string s;
            cin>>s;
            REP(j,s.sz) damage[i][j]=toDigit(s[j]);
        }
        vci wep;
        MS(dp,-1);
        printf("Case %d: %d\n",++T,bitmask(0,wep));
