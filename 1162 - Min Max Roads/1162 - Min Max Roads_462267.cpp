...
int Q[MAX][40],A[MAX],Qmin[MAX][40];
 
 
...
 
    REP(i,n)
        P[i][0]=T[i],Q[i][0]=Qmin[i][0]=A[i];
 
    for(int j=1; (1<<j)<n; j++)
        REP(i,n)
            if(P[i][j-1]!=-1)
                P[i][j]=P[P[i][j-1]][j-1],Q[i][j]=max(Q[i][j-1],Q[P[i][j-1]][j-1]),Qmin[i][j]=min(Qmin[i][j-1],Qmin[P[i][j-1]][j-1]);
}
 
pri LCA(int p,int q){
    int x;
...
    for(int i=x; i>=0; i--)
        if(L[p]-(1<<i)>=L[q])
            ret=max(ret, Q[p][i]),ret2=min(ret2, Qmin[p][i]),p=P[p][i];
 
    if(p==q) return mp(ret,ret2);
 
    for(int i=x; i>=0; i--)
        if(P[p][i]!=-1 && P[p][i]!=P[q][i])
            ret2=min(ret2, Qmin[p][i]),ret2=min(ret2, Qmin[q][i]),ret=max(ret, Q[p][i]),ret=max(ret, Q[q][i]),p=P[p][i],q=P[q][i];
 
    ret=max(ret,A[p]);
    ret=max(ret,A[q]);
    ret2=min(ret2,A[p]);
