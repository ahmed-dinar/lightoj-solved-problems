...
    N=1;
    prime.pb(2);
    for(int i=3; i<MAX; i+=2) if(flag[i]==0) prime.pb((i64)i),N++;
}
 
void prime_fact(i64 n,vector<pair<i64,int> >& v){
    for(int i=0; prime[i]*prime[i]<=n && i<N ; i++){
        if(n%prime[i]==0){
            int p=0;
...
 
            REP(i,y.sz){
                if( nfound(x,y[i]) ) REP(j,y[i].ss) ans*=y[i].fi;
            }
 
            printf("%lld\n",ans);
            //nl;
