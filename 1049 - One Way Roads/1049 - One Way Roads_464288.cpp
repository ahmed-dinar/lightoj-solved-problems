...
template<class T>T Pow(T n,T p){ if(p==0) return (T)1; if(p&1) return n*Pow(n,p-1);  else return sq(Pow(n,p/(T)2));}
...
 
vector<pri>g[MAX];
int cost[2],vis[MAX][3];
 
void dfs(int u,int d,int p,int k){
    cost[k]+=d;
    REP(i,g[u].sz){
        int v=g[u][i].fi;
        if(v==p) continue;
